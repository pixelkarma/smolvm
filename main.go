...
func main() {
	...
	flag.StringVar(&cfg.ShelleyBinaryPath, "shelley-binary", envOrDefault("SMOLVM_SHELLEY_BINARY", "./shelley-linux-aarch64"), "Path to shelley runtime binary used to build instance image")
	flag.StringVar(&cfg.ImageName, "image", envOrDefault("SMOLVM_IMAGE", "smolvm-shelley:latest"), "Docker image name for managed instances")
